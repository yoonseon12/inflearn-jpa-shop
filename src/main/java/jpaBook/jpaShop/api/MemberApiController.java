...
public class MemberApiController {
    ...
    /**
     * 장점 :
     *  1. 엔티티와 API스택을 명확하게 분리하기위해  별도의 DTO를 사용하여 엔티티를 변경해도 API 스택이 전혀 바뀌지 않는다.
     *  2. API의 request 항목이 무엇인지 정리가 된다.
     *  3. API 스택에 맞게 별도의 validation 체크가 가능하다. -> 유지보수에 용이
     *     (엔티티 자체에 @NotEmpty를 넣을 경우 API마다 validation 체크가 다를 경우 사이드 임펙트 발생)
     */
